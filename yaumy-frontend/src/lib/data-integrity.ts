...
export async function repairClipsMissingChunks(projectId: string) {
  ...
  // Get all clips for the project and filter out those missing chunks in JavaScript
  ...
  // Filter clips that have null or undefined chunks
  const brokenClips = allClips.filter((clip) => !clip.chunks);
